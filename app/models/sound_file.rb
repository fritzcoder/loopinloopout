...
class SoundFile < ActiveRecord::Base
  ...
  attr_accessible :name, :description, :length, :file, :preview, :bpm, :type, :track_part
  ...
  def categories
    types = SoundFileType.find(:all, :conditions => {:sound_file_id => id})
    types.map {|x| x.sound_type}
  end
end
